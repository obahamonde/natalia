...
const wsUrl = computed(() => {
  if (!state.currentConversation) return;
  return "wss://api.aiofauna.com/api/ws?namespace=" + state.currentConversation.ref;
});

</script>
<template>
  <Icon icon="mdi-menu" class="fixed top-6 left-6 text-black dark:text-white x2 scale cp" @click="show = !show"
    v-if="!show" />
  
  <transition name="fade-slide">
    
    <div class="tl fixed m-4 h-full w-72 bg-gray-700 dark:bg-gray-500 z-50" v-if="show">
      <h1 class="w-full px-4 py-2 text-center">
        <Icon icon="mdi-menu" class="scale cp x2 text-black dark:text-white" @click="show = !show" />
      </h1>

      
      <button class="row center bg-secondary px-2 m-4 rounded sh scale cp" @click="createConversation()">
        <Icon icon="mdi-plus" class="top-12 left-0 m-2 x2 scale cp" /><span class="m-4 text-lg">New Conversation</span>
      </button>
      <div v-for="i in state.conversations" class="mt-16 m-4 col center" v-if="state">
        <Icon icon="mdi-delete" class="right-4 absolute text-primary hover:text-error cp scale"
          @click="handleDelete(i.ref)" />
        <dibutton
          :class="(state.currentConversation && (i.ref == state.currentConversation!.ref)) ? 'bg-accent text-white rounded-lg sh' : ''"
          class="text-lg text-center p-4 cp" @click.prevent="fetchConversation(i.ref)">
          {{ i.ref == state.currentConversation?.ref ? state.currentConversation.title : i.title }}
        </dibutton>
      </div>
    </div>
  </transition>
  ...
  <transition>
    <div class="col center mx auto">
      ...
      <input v-model="message"
        ...
        placeholder="Enter your message here" text-light dark:text-accent @keyup.enter="sendWsMessage()" />
    </div>
  </transition>



  <Jarvis @speech="receiveAudio($event)" @finished="sendAudio" class="scale-50" />
</template>
