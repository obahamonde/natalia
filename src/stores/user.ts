...
import { User, Notification, Conversations, ChatMessage } from "../types";
...
export const useStore = defineStore("state", () => {
  const state = reactive({
    ...
    currentConversation: null as Conversations | null,
  });
  ...
  const fetchMessages = async (reference: string, message:string) => {
    ...
    length ?  await getMessages(reference, message) : await fetchTitle(reference, message);
  };
  return {
    chunkify,
    state,
